...
function fnopar(t, y, ydot, user_data)
  y = Sundials.asarray(y)
  ydot = Sundials.asarray(ydot)
...
#= Set Jacobian =#
flag = Sundials.CVDlsSetDenseJacFn(cvode_mem, Jac)  # works, but clunky, see above and cvode_Roberts_dns.jl
...
while iout<=nout
  ...
  if sensi
    ...
    PrintOutputS(yS)
    #println("Sens1: ", Sundials.asarray(tempyS[1]))
    #println("Sens2: ", Sundials.asarray(tempyS[2]))
    #println("Sens3: ", Sundials.asarray(tempyS[3]))
    #println()
  end
  @printf("-----------------------------------------");
  @printf("------------------------------\n");
