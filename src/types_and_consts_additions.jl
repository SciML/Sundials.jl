...
CVRhsFn_wrapper(f) = local_cfunction(f,Cint,(realtype,N_Vector,N_Vector,Ptr{Cvoid}))

ARKRhsFn_wrapper(fp::ARKRhsFn) = fp
ARKRhsFn_wrapper(f) = local_cfunction(f,Cint,(realtype,N_Vector,N_Vector,Ptr{Cvoid}))

CVRootFn_wrapper(fp::CVRootFn) = fp
CVRootFn_wrapper(f) = local_cfunction(f,Cint,(realtype,N_Vector,Ptr{realtype},Ptr{Cvoid}))

CVQuadRhsFn_wrapper(fp::CVQuadRhsFn) = fp
CVQuadRhsFn_wrapper(f) = local_cfunction(f,Cint,(realtype,N_Vector,N_Vector,Ptr{Cvoid}))

IDAResFn_wrapper(fp::IDAResFn) = fp
IDAResFn_wrapper(f) = local_cfunction(f,Cint,(realtype,N_Vector,N_Vector,N_Vector,Ptr{Cvoid}))

IDARootFn_wrapper(fp::IDARootFn) = fp
IDARootFn_wrapper(f) = local_cfunction(f,Cint,(realtype,N_Vector,N_Vector,Ptr{realtype},Ptr{Cvoid}))

KINSysFn_wrapper(fp::KINSysFn) = fp
KINSysFn_wrapper(f) = local_cfunction(f,Cint,(N_Vector,N_Vector,Ptr{Cvoid}))
