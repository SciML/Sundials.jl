function solve{uType,tType,isinplace,algType<:SundialsAlgorithm,F}(prob::AbstractODEProblem{uType,tType,Val{isinplace},F},
    ...
    try
        ...
        flag = Sundials.@checkflag Sundials.CVodeInit(mem, cfunction(Sundials.cvodefun, Cint, (Sundials.realtype, Sundials.N_Vector, Sundials.N_Vector, Ref{typeof(userfun)})), t0, convert(Sundials.N_Vector, u0nv))
        ...
        if save_timeseries
          for k in 2:length(save_ts)
              ...
              while tout[end] < save_ts[k]
                  looped = true
                  flag = Sundials.@checkflag Sundials.CVode(mem, save_ts[k], u, tout, Sundials.CV_ONE_STEP)
                  push!(ures,copy(u))
                  push!(ts, tout...)
              end
              if looped
                  # Fix the end
                  flag = Sundials.@checkflag Sundials.CVodeGetDky(mem, save_ts[k], Cint(0), ures[end])
                  ts[end] = save_ts[k]
