...
DiffEqBase.reeval_internals_due_to_modification!(integrator::AbstractSundialsIntegrator) =
    nothing
DiffEqBase.reeval_internals_due_to_modification!(integrator::IDAIntegrator) =
...
function DiffEqBase.initialize_dae!(integrator::IDAIntegrator)
    ...
    if any(abs.(integrator.tmp) .>= integrator.opts.reltol)
        ...
        if integrator.alg.init_all
            ...
            integrator.flag =
                IDASetId(integrator.mem, integrator.sol.prob.differential_vars)
        end
