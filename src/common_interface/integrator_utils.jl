...
function DiffEqBase.savevalues!(
    ...
    while !isempty(integrator.opts.saveat) &&
        ...
        if integrator.opts.dense
            ...
            save_value!(
                integrator.sol.interp.du, tmp, uType,
                integrator.opts.save_idxs, false
            )
        end
    end
...
function save_value!(save_array, val, ::Type{<:Number}, save_idxs, make_copy::Bool = true)
    return push!(save_array, first(val))
end
function save_value!(
        save_array,
        val,
    ...
    return push!(save_array, save)
end

function handle_callback_modifiers!(integrator::CVODEIntegrator)
    return CVodeReInit(integrator.mem, integrator.t, integrator.u_nvec)
end

# Dispatch for ARKStep (implicit methods)
function handle_callback_modifiers!(
...
function DiffEqBase.reeval_internals_due_to_modification!(
    ...
    integrator.userfun.p = integrator.p
    return handle_callback_modifiers!(integrator, callback_initializealg)
end
...
function DiffEqBase.get_tstops_max(integ::AbstractSundialsIntegrator)
    return maximum(get_tstops_array(integ))
end
